...
if flag == 'local':
    ...
    username = parser.get('ATFARM','username')
    pwd = parser.get('ATFARM','pwd')
    source = parser.get('ATFARM','source_details')
    source= json.loads(source)
    print(source)
    jdbc_driver_name = parser.get('ATFARM','jdbc_driver_name')
    jdbc_driver_loc = parser.get('ATFARM','jdbc_driver_loc')
    connection_string = parser.get('ATFARM','connection_string')
...
def fetch_tables(con,db,all=False):
    if all:
        ...
        tables = ["m_organisations","m_organisations_users","m_users"]
    return tables
...
def get_tabular_frame(con,table,ffdp_data_metrics,db,client):
    if ffdp_data_metrics['row_count'][0] == 0:
        ...
        query = """SELECT * FROM "{}".{} where created >= '{}' or updated >= '{}' """.format(test_schema,table,latest_created,latest_updated)
...
def get_source_connection(db):
    if flag == 'local':
        ...
        con = psycopg2.connect(user = username,
                                    ...
                                    #port = "5439",
                                    database = database)
    print(f'source connection established')
    return con
...
def get_table_metrics(table_name,con,db,type):
    if type =='ffdp':
        query = f"""select date_trunc('second',max(created)) as latest_created, date_trunc('second',max(updated)) as latest_updated, count(*) as row_count from {prefix}_{test_schema}_{db}.{table_name};"""
        ...
        query = f"""select date_trunc('second',max(created)) as latest_created, date_trunc('second',max(updated)) as latest_updated, count(*) as row_count from {test_schema}.{table_name};"""
    # print(query)
    df = pd.read_sql(query,con)
    df = df.drop_duplicates() # Droping duplicates in the legacy db if any
...
def fetch_and_push_core_db(force_load):
    db = 'core'
...
def db_to_s3_func(force_load=False):
    fetch_and_push_core_db(force_load)
